...
for k = 1:num_segments
    ...
    % Minimum distance required to reach v_max and decelerate back to 0
    d_crit = (v_max^2) / a_max;
    
    if D >= d_crit
        % Case 1: Long move (Trapezoidal profile)
        % The robot has enough distance to accelerate, reach v_max, and decelerate.
        % Time = (Time to accelerate/decelerate) + (Time at cruise speed)
        T_seg(k) = (v_max / a_max) + (D / v_max);
    else
        % Case 2: Short move (Triangular profile)
        % The move is so short the robot must start decelerating before it ever hits v_max.
        T_seg(k) = 2 * sqrt(D / a_max);
    end
    
    % Optional: Add a safety factor if the cubic spline overshoots the trapezoidal estimate
    % T_seg(k) = T_seg(k) * 1.1; 
end
